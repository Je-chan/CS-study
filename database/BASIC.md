...
- 개념, 내부 매핑은 개념 스키마와 내부 스키마의 대응 관계를 정의한 것으로, "저장 인터페이스" 라고 한다

## 3-4) 데이터 사전
- 데이터베이스가 저장된 데이터를 올바르게 관리하고 이용하기 위해 필요한 부가 정보들도 저장
- 대표적인 부가 정보가 스키마와 매핑(사상) 정보
- 데이터베이스에 저장되는 데이터에 관한 정보를 저장하는 곳을 데이터 사전 혹은 시스템 카탈로그라고 한다
- 어떤 곳에서는 시스템 데이터베이스라고도 한다(이에 파생해서 우리가 사용하는 일반 데이터베이스를 사용자 데이터베이스라고도 부름

### 데이터 사전에 저장되는 것
- 일반 사전터럼 데이터베이스에 저장되어 있는 데이터를 정확하고 효율적으로 이용하기 위해 참고해야 하는 스키마, 사상 정보, 다양한 제약 조건 등을 지정
- 데이터베이스에 저장되는 데이터에 관한 정보라 "데이터를 위한 데이터", 즉 "메타 데이터" 라고 부른다
...
### 데이터 정의어 (DDL)
- 새로운 데이터베이스를 구축하기 위한 언어
- 스키마 구조와 제약조건 등을 정의, 삭제, 수정한다
- 데이터 정의어로 정의된 스키마는 데이터 사전에 저장되고 삭제나 수정이 발생하면 데이터 사전에도 반영된다
- create table, alter table 등Í
        
### 데이터 조작어 (DML)
- 데이터베이스 관리 시스템에 요구를 하기 위해 사용하는 데이터 언어
- 사용자가 실제 데이터 값을 활용하기 위해 사용하는 것이 데이터 조작어
- 데이터를 삽입, 삭제, 수정
- Insert, Delete, Update 등
- 데이터를 검색하는 데이터 질의어 (Select) 도 큰 범위 안에서 여기에 포함시키기도 한다

### 절차적 데이터 조작어 (Procedural DML)
- 사용자가 어떤 데이터를 원하고 해당 데이터를 얻으려면 어떻게 처리해야 하는지를 구체적으로 설명하는 방식

### 비절차적 데이터 조작어 (Nonprocedural DML)
- 사용자가 어떤 데이터를 원하지만 설명하는 방식
- 해당 데이터를 얻기 위해 어떻게 처리해야 하는지는 데이터베이스 관리 시스템에 전적으로 맡긴다
- 선언적 언어라고도 한다
        
### 데이터 제어어 (DCL)
- 데이터베이스에 저장된 데이터를 여러 사용자가 무결성과 일관성을 유지하며 문제 없이 공유하기 위해 사용하는 언어
- 즉, 내부적으로 필요한 규칙(보안, 무결성 체크) 등을 정의하는 데이터 언어
- 백업, 백업 이미지로 회복하는 명령어 등
- 데이터 제어어를 이용해 규칙이나 기법을 정의하는 이유는 아래의 특성들을 보장하기 위함. 이 특성들이 DBMS 의 장점이다
1. 무결성 : 데이터베이스에 정확하고 유효한 데이터만을 유지
2. 보안 : 허가 받지 않는 사용자가 데이터에 접근하는 것을 차단하거나, 허가된 사용자가 접근 권한이 있는 데이터에만 접근할 수 있도록 하는 것
3. 회복 : 장애가 발생해도 데이터 일관성 유지 
4. 동시성 : 여러 사용자가 같은 데이터에 동시 접근하여 처리할 수 있도록 함
